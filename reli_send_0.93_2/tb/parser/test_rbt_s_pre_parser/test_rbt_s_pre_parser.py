...
InputBus, InputTransaction, InputSource, InputSink, InputMonitor = define_stream("Input",
    signals=["valid", "ready", "data", "length", "meta" ,"pktlen","tuser"]
)
...
class TB:
    ...
    async def send(self, pkt):
        ...
        mata_val = BinaryValue(n_bits=32, bigEndian=False)
        for i in range(4):
            mata_val[i*8+7:i*8] = 0x00 
            
        user_width=int(os.getenv("PARAM_USER_WIDTH", "56"))
        
        tuser_val = BinaryValue(n_bits=user_width, bigEndian=False)
        for i in range(7):
            if(i==0):
                tuser_val[i*8+7:i*8] = 0xd0
            else:
                tuser_val[i*8+7:i*8] = 0x00
                    

        inputTransaction.data = data_val
        inputTransaction.length = 74
        inputTransaction.pktlen = len(pkt)
        inputTransaction.meta = mata_val
        inputTransaction.tuser = tuser_val
...
async def run_test_full(dut, payload_lengths=None, payload_data=None, idle_inserter=None, backpressure_inserter=None):
    ...
    for payload in [payload_data(x) for x in payload_lengths()]:
        ...
        phv_h_out[PKTLEN_NO]=0

        phv_b_out[PKT_PROPERTY_NO]=0x0
        phv_b_out[PHV_IN_PORT_NO]=0xd0
        phv_b_out[PHV_OUT_PORT_NO]=0x0
        phv_b_out[PHV_TID_NO]=0x0
        phv_b_out[STL_OFFSET_NO]=0x0
...
@pytest.mark.parametrize("tuser_width", [56])
